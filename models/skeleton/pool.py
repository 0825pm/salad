...
class STPool(nn.Module):
    ...
    def __init__(
        ...
        dataset="t2m",
        depth=0,
        skeleton_mode="7part",  ## PATCH: skeleton mode for sign
    ):
        ## ── PATCH: add "sign" ──
        if not dataset in ["t2m", "kit", "sign"]:
            raise ValueError("dataset should be 't2m', 'kit', or 'sign'")
        ## ── end PATCH ──
        
        super(STPool, self).__init__()

        self.skeleton_pool, self.skeleton_mapping, self.new_edges = self._get_skeleton_pooling(dataset, depth, skeleton_mode)
    ...
    def _get_skeleton_pooling(self, dataset, depth, skeleton_mode="7part"):
        if depth == 0:
            if dataset == "t2m":
            ...
            ## ── PATCH: sign skeleton (identity pooling, mode-aware) ──
            elif dataset == "sign":
                from utils.sign_paramUtil import get_sign_config
                _, adj_list, _, num_j = get_sign_config(skeleton_mode)
                weight = torch.zeros(num_j, num_j)
                mapping = [[(i,), i] for i in range(num_j)]
                new_edges = adj_list_to_edges(adj_list)
                for joints, idx in mapping:
                    weight[idx, list(joints)] = 1
                weight = weight / weight.sum(dim=1, keepdim=True)
                return weight, mapping, new_edges
            ## ── end PATCH ──

            if dataset in ["t2m", "kit"]:
            ...
            ## ── PATCH: sign at depth 1 (identity, mode-aware) ──
            if dataset == "sign":
                from utils.sign_paramUtil import get_sign_config
                _, adj_list, _, num_j = get_sign_config(skeleton_mode)
                weight = torch.zeros(num_j, num_j)
...
class STUnpool(nn.Module):
    ...
    def _get_skeleton_unpool(self, skeleton_mapping):
        ...
        for joints, idx in skeleton_mapping:
            weight[list(joints), idx] = 1
            
        return weight
    
    def forward(self, x):
        B, T, J_in, D = x.size()
        out = torch.matmul(self.skeleton_unpool, x)
        J_out = out.size(2)
        out = out.permute(0, 2, 3, 1).reshape(B * J_out, D, T)
