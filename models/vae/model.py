...
from models.vae.encdec import (
    ...
    SignMotionEncoder, SignMotionDecoder,
...
class VAE(nn.Module):
    def __init__(self, opt):
        ...
        if getattr(opt, 'dataset_name', 't2m') == 'sign':
            ...
            use_hand_vel = getattr(opt, 'use_hand_vel', True)
            _, _, _, num_parts = get_sign_config(
                getattr(opt, 'skeleton_mode', '7part'), use_hand_vel)
        else:
